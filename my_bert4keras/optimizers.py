        ...
        def get_updates(self, loss, params):
            ...
            def new_update(x, new_x):
                if is_one_of(x, params) and self._do_layer_adaptation(x):
                    ...
                    ratio = K.switch(
                        x_norm > 0.0, K.switch(g_norm > 0.0, x_norm / g_norm, 1.0), 1.0
                    )
                    new_x = x + dx * ratio
                return old_update(x, new_x)
        ...
        def get_gradients(self, loss, params):
            ...
            for p in params:
                if p not in self.accum_grads:
                    self.accum_grads[p] = K.zeros(K.int_shape(p), dtype=K.dtype(p))
...
def extend_with_lookahead_v2(BaseOptimizer):
    ...
    class NewOptimizer(BaseOptimizer):
        ...
        def _resource_apply(self, grad, var, indices=None):
            ...
            with tf.control_dependencies([op]):
                slow_update = K.update(slow_var, K.switch(cond, slow_var_t, slow_var))
...
def extend_with_exponential_moving_average_v2(BaseOptimizer):
    ...
    class NewOptimizer(BaseOptimizer):
        ...
        def _resource_apply_dense(self, grad, var):
            ...
            ema_momentum = self.ema_momentum
            with tf.control_dependencies([op]):
                return K.update(ema, ema * ema_momentum + var * (1.0 - ema_momentum))

        def _resource_apply_sparse(self, grad, var, indices):
            op = super(NewOptimizer, self)._resource_apply_sparse(grad, var, indices)
            ema = self.get_slot(var, "ema")
            ema_momentum = self.ema_momentum
            with tf.control_dependencies([op]):
                return K.update(ema, ema * ema_momentum + var * (1.0 - ema_momentum))
